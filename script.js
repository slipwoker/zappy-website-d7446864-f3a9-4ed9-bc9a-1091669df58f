document.addEventListener('DOMContentLoaded', function () {
  ...
  if (contactForm) {
    ...
    function getFieldLabel(field) {
      ...
      if (!label) {
        label = field.closest('.form-group, .field-wrapper, div')
          ? field.closest('.form-group, .field-wrapper, div').querySelector('label') : null;
      }
      return label ? label.textContent.trim().replace('*', '').trim() : field.name || field.type;
    }
    ...
    function validateField(field) {
      ...
      if (pattern) {
        ...
        if (!regex.test(value)) {
          ...
          showError(field, title || label + ' format is invalid.');
          return false;
        }
      }
    ...
    contactForm.addEventListener('submit', function (e) {
      ...
      fields.forEach(function (field) {
        if (field.type === 'hidden' || field.type === 'submit' || field.type === 'button' || field.type === 'reset') return;
      ...
      if (!isFormValid) {
        ...
        if (firstInvalid) {
          ...
          const navbarEl = document.querySelector('.navbar, nav, header');
          const navbarHeight = navbarEl ? navbarEl.offsetHeight : 0;
          const elementTop = firstInvalid.getBoundingClientRect().top + window.pageYOffset;
          window.scrollTo({
      ...
      fetch(action, {
        ...
        .then(function () {
          ...
          const errorEls = contactForm.querySelectorAll('.field-error');
...
(function() {
    ...
    contactForm.addEventListener('submit', async function(e) {
        ...
        try {
            ...
            var payload = {
                websiteId: 'd7446864-f3a9-4ed9-bc9a-1091669df58f',
                name: resolvedName,
                email: resolvedEmail,
            ...
            if (result.success) {
                ...
                if (result.thankYouPagePath && result.ticketNumber) {
                    ...
                    if (isPreview && window.ZAPPY_CONFIG) {
                        var wid = window.ZAPPY_CONFIG.websiteId || 'd7446864-f3a9-4ed9-bc9a-1091669df58f';
...
(function(){
  try {
    ...
    document.addEventListener('submit', function(e) {
      ...
      var wid = 'd7446864-f3a9-4ed9-bc9a-1091669df58f';
